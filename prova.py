...
# PER PYTHONANYWHERE: Bisogna cambiarla oppure usare un'altra funzione Play
...
def record(seconds=3, sample_rate=DEFAULT_SAMPLE_RATE, normalize_db=0.1):
  ...
  #audio_string = output.eval_js('record(%d)' % (seconds * 1000.0)) NON FUNZIONA FUORI COLAB
...
# PER PYTHONANYWHERE: DA MODIFICARE
...
record_or_upload = "Record"  #@param ["Upload (.mp3 or .wav)","Record"]

record_seconds =     12#@param {type:"number", min:1, max:25, step:1}
...
import sounddevice as sd
# %%
#Source Separation & Audio Features Extraction
...
if __name__ == "__main__":
  ...
  separator.separate_to_file('Brani Prova\celeste.wav', 'Stems')

  #write("Brani Prova\Fly Me - Cut.wav", 16000, audio.T)

  vocals, sr = librosa.load('Stems\celeste/vocals.wav', sr=16000)
  other, sr = librosa.load('Stems\celeste/other.wav', sr=16000)
  drums, sr = librosa.load('Stems\celeste/drums.wav', sr=16000)
  bass, sr = librosa.load('Stems\celeste/bass.wav', sr=16000)
