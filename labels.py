...
def extract_label_13_inst(label_path, m_beat_arr):
    ...
    inst_ary_out = np.zeros([len(label), 13]).astype(np.float32)
    inst_ary_out[:, 0] = np.max(label[:, [33]], axis=1)  # Bass drum
    inst_ary_out[:, 1] = np.max(label[:, [35]], axis=1)  # Snare drum
    ...
    inst_ary_out[:, 7] = np.max(label[:, [33]], axis=1)  # low-tom
    inst_ary_out[:, 8] = np.max(label[:, [35]], axis=1)  # mid-tom
    inst_ary_out[:, 9] = np.max(label[:, [35]], axis=1)  # high-tom
    inst_ary_out[:, 10] = np.max(label[:, [39]], axis=1)  # Crash
    inst_ary_out[:, 11] = np.max(label[:, [39]], axis=1)  # Ride
    inst_ary_out[:, 12] = np.max(label[:, [37]], axis=1)  # Maracas
